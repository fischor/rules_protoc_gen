...
| predeclared_outputs |  If true, the plugin is configured to accept a list of predeclared outputs when used in a <code>protoc_output</code> target. The list of predeclared outputs is passed in the <code>outputs</code> attribute of the <code>protoc_output</code> rule. See also the documention of <code>protoc_plugin</code>.   | Boolean | optional | False |
